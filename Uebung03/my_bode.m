...
%       sub = 2 in {2; inf}             maximum number of subplots
%       w = logspace(-1, 6, 100)
...
%                 y(t) = C * x(t) + D * u(t)
%
%   It plots |G(iw)|_{dB20} and \phi(G(iw) over log10(w) in the current
%   figure. If desired, call "hold on" BEFORE using my_bode(..)
...
function my_bode(A, B, C, varargin)
    ...
    addParameter(ip, 'sub',  2,                     @isnumeric);    % max number of subplots on figure
